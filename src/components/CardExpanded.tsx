...
export const CardExpanded : React.FC<cardExpandedProps>= ({ url, title, explanation, copyright }) => {
  ...
  return (  
    <article className='w-full h-full border-2 rounded-md border-slate-900 shadow-lg bg-slate-100 p-3 md:p-5 grid grid-cols-1 md:grid-cols-2 grid-rows-12 relative'>
      ...
      <small className={`w-auto h-auto md:flex md:items-center max-h-full text-xs lg:text-base antialiased font-extralight text-justify col-span-1 md:col-span-2 ${ isExpanded ? 'row-start-2 overflow-auto' : 'row-start-7 overflow-hidden' } row-end-13 md:row-start-2 md:row-end-13 md:px-5 relative mt-2 md:mt-0`}>
        { explanation }
